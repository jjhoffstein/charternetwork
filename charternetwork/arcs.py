...
from charternetwork.config import DUTY_LIMIT_HRS, FLIGHT_LIMIT_HRS, FUEL_PRICE_GAL


def _ferry_cost(f_hrs, tail):
    "Total ferry cost: hourly charter rate + fuel burn"
    return f_hrs * tail.hourly_rate + f_hrs * tail.fuel_burn_gal_hr * FUEL_PRICE_GAL


def is_feasible(trip_a, trip_b, tail, apts, max_ferry_nm=500, max_gap_hrs=48):
    "Can a tail serve trip_b after completing trip_a?"
    # Capacity check
    if tail.capacity < max(trip_a.party_size, trip_b.party_size):
        return False

    # Ferry distance check
    ferry_nm = leg_dist(trip_a.home_apt, trip_b.home_apt, apts) if trip_a.home_apt != trip_b.home_apt else 0.0
    if ferry_nm > max_ferry_nm:
        return False

    f_hrs = ferry_nm / tail.cruise_kt

    # T1.1: Ferry flight must not exceed crew flight-time limit
    if f_hrs > FLIGHT_LIMIT_HRS:
        return False
    ...
    if earliest > trip_b.depart_out:
        return False

    # T1.1: Crew duty window check — ferry + trip_b outbound leg must fit in duty limit
    trip_b_flight_hrs = trip_b.dist_nm / tail.cruise_kt
    duty_hrs = f_hrs + trip_b_flight_hrs + tail.turnaround_hrs
    if duty_hrs > DUTY_LIMIT_HRS:
        return False
...
def make_arc(trip_a, trip_b, tail, apts):
    ...
    return Arc(from_trip=trip_a.id, to_trip=trip_b.id, ferry_origin=trip_a.home_apt,
        ferry_dest=trip_b.home_apt, ferry_nm=f_nm, ferry_hrs=f_hrs,
        ferry_cost=_ferry_cost(f_hrs, tail))
...
def depot_out(tail, trip, apts):
    ...
    return Arc(from_trip=None, to_trip=trip.id, ferry_origin=tail.home_base,
        ferry_dest=trip.home_apt, ferry_nm=f_nm, ferry_hrs=f_hrs,
        ferry_cost=_ferry_cost(f_hrs, tail))
...
def depot_in(trip, tail, apts):
    ...
    return Arc(from_trip=trip.id, to_trip=None, ferry_origin=trip.home_apt,
        ferry_dest=tail.home_base, ferry_nm=f_nm, ferry_hrs=f_hrs,
        ferry_cost=_ferry_cost(f_hrs, tail))
    ...
    for tail in fleet:
        ...
        for i, ta in enumerate(sorted_trips):
            for tb in sorted_trips[i+1:]:
                if (tb.depart_out - ta.arrive_home).total_seconds()/3600 > max_gap_hrs:
                    break
